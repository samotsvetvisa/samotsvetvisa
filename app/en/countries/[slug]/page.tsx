    ...
    <>
      ...
      <main>
        ...
        <section className="section-shell sources-block">
          <div><p className="eyebrow">Primary sources</p><h2>{item.headings.sources}</h2><p>{item.headings.sourcesNote}</p></div>
        ...
        <section className="section-shell closing-cta country-cta">
          ...
          <div><p>{item.headings.closingBody}</p><Link className="button button-gold" href={withTrailingSlash(`/en/assessment?country=${item.slug}`)}>Get a profile assessment</Link></div>
        </section>
      </main>
