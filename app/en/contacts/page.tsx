  ...
  return (
    <><SiteHeader locale="en" /><main>
      ...
      <section className="section-shell contacts-layout">
        <article className="contact-card contact-card-primary"><p className="eyebrow eyebrow-light">Still have questions?</p><h2>Contact us</h2><p>Send a short outline by Telegram or email. We will reply personally and propose a practical next step.</p><div className="contact-links"><a className="button contact-channel contact-channel-telegram" href={TELEGRAM_DIRECT_URL} target="_blank" rel="noreferrer" data-contact-channel="telegram" data-cta-location="contacts_primary"><TelegramIcon /><span>Message us on Telegram</span></a><a className="button contact-channel" href={`mailto:${CONTACT_EMAIL}`} data-contact-channel="email" data-cta-location="contacts_primary">Send an email</a><a className="contact-channel-link" href={TELEGRAM_URL} target="_blank" rel="noreferrer" data-contact-channel="telegram_channel" data-cta-location="contacts_primary">Open the channel {TELEGRAM_HANDLE}</a></div></article>
        <article className="contact-card"><p className="eyebrow">Initial enquiry</p><h2>Book a free consultation</h2><p>Answer five short questions. We will contact you within one working day to arrange a conversation with Nikita Samotsvetov.</p><ConsultationLink className="button button-primary" locale="en" location="contacts_card" /></article>
        ...
        <article className="contact-card"><p className="eyebrow">Data requests</p><h2>How to submit a request</h2><p>A request for access, correction, restriction or deletion may be sent by email. Include your name and the contact method used in the form so that we can locate the record.</p><a className="text-link" href={`mailto:${CONTACT_EMAIL}`} data-contact-channel="email" data-cta-location="privacy_request">Submit an electronic request <span aria-hidden="true">↗</span></a></article>
