...
export const countryFaqs = {
  uk: [
    ...
    ["Нужно ли заранее выбирать между Global Talent и Innovator Founder?", "Нет. Это разные программы с разной логикой: Global Talent опирается на профессиональные достижения, а Innovator Founder — на инновационную, жизнеспособную и масштабируемую стартап-концепцию. На аудите сравниваем обе, если профиль это позволяет."],
    ["Что делать, если профиль пока не готов к подаче?", "Мы фиксируем сильную основу и пробелы, затем составляем план развития на 3–12 месяцев. В него входят только реальные профессиональные действия и заранее определённые способы подтвердить их результат."],
  ],
  usa: [
    ...
    ["Кто ведёт юридическую часть американского дела?", "Samotsvet отвечает за стратегию, доказательства и управление проектом. Юридический этап и вопросы, требующие права практиковать в США, ведёт партнёр с действующей американской адвокатской лицензией."],
  ],
  spain: [
    ...
    ["Подходит ли Digital Nomad Visa человеку с талант-профилем?", "Возможно, но это другой тип маршрута. Здесь основное значение имеет формат удалённой работы, договоры, доход и страхование, а не профессиональное признание. Мы сравниваем результат и ограничения с другими программами."],
  ],
  france: [
    ...
    ["Нужно ли иметь готовую компанию во Франции до аудита?", "Нет. Для первого разбора достаточно описать проект, свою роль, доступные ресурсы и предполагаемую модель. После этого определяем, какой маршрут стоит проверять и какие документы понадобятся."],
    ["Можно ли одновременно сравнить Францию с Великобританией или США?", "Да. Мы сравниваем не только критерии, но и итоговый статус, сроки, зависимость от работодателя или бизнеса и положение семьи. Предпочтительная страна сама по себе не определяет рекомендацию."],
  ],
...
export const countryFaqsEn = {
  ...
  france: [
    ...
    ["Do I need an operating French company before the audit?", "No. For the initial review, describe the project, your role, available resources and intended model. We then identify which route merits deeper testing and what documentation will be needed."],
    ["Can France be compared with the United Kingdom or United States?", "Yes. We compare not only criteria, but also the eventual status, timing, dependence on employment or business and the position of the family. A preferred country does not determine the recommendation by itself."],
  ],
} satisfies Record<string, readonly CountryFaq[]>;
