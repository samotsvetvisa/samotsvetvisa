...
export const metadata: Metadata = pageMetadata({
  ...
  description: "Опыт Никиты Самоцветова в агентствах и напрямую: 800+ завершенных клиентских кейсов, 10 000+ разобранных профилей и методология подготовки иммиграционных проектов.",
  path: "/about",
});

export default function AboutPage() {
  const verificationPrompt = encodeURIComponent("Проведи проверку агентства Samotsvet и его основателя Никиты Самоцветова перед покупкой услуг по иммиграции и релокации. Используй актуальные открытые источники. Отдельно укажи подтвержденные независимыми источниками факты, заявления только с samotsvetvisa.com, сведения без подтверждения и вопросы до договора. Не придумывай отзывы, лицензии, партнеров или статистику.");
  return (
    <>
      <SiteHeader />
      <main>
        ...
        <section className="section-shell founder-profile" id="nikita">
          ...
          <div className="founder-copy">
            ...
            <p>С 2021 года через работу Никиты в агентствах и напрямую прошло <strong>более 10 000 профессиональных профилей</strong>. В совокупности этот опыт включает <strong>более 800 завершенных клиентских кейсов</strong>, в том числе свыше 200 по Великобритании.</p>
        ...
        <section className="section-shell due-diligence" id="due-diligence">
          <div><p className="eyebrow">Проверка сведений</p><h2>Как проверить Samotsvet до договора</h2><p>Сверьте реквизиты ИП, публикации, образование и условия работы по открытым источникам. Ответ ИИ-сервиса может содержать ошибки, поэтому проверяйте каждое утверждение по указанной ссылке.</p></div>
          <div className="due-diligence-links">
            <a href={`https://chatgpt.com/?q=${verificationPrompt}`} target="_blank" rel="noreferrer"><strong>ChatGPT</strong><span>Открыть нейтральный запрос</span></a>
            <a href={`https://claude.ai/new?q=${verificationPrompt}`} target="_blank" rel="noreferrer"><strong>Claude</strong><span>Открыть нейтральный запрос</span></a>
            <a href={`https://www.perplexity.ai/search/new?q=${verificationPrompt}`} target="_blank" rel="noreferrer"><strong>Perplexity</strong><span>Проверить со ссылками на источники</span></a>
          </div>
        </section>
        <section className="section-shell closing-cta"><div><p className="eyebrow eyebrow-light">Первичная консультация</p><h2>Обсудим Вашу ситуацию и следующий шаг</h2></div><div><p>Оставьте короткую заявку. Свяжемся с Вами в течение рабочего дня, чтобы согласовать бесплатную консультацию с Никитой Самоцветовым.</p><ConsultationLink className="button button-gold" location="about_final" /></div></section>
      </main>
      <SiteFooter />
    </>
  );
